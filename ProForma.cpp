...
namespace OpenMS
{
    ...
    void ProForma::parseCVModificationNames(const std::string& modString, size_t& pos, size_t residue_pos)
    {
        ...
        // Split the modification string by commas to handle multiple positions
        std::istringstream mod_stream(modification);
        std::string mod_part;
        while (std::getline(mod_stream, mod_part, ','))
        {
            // Validate each part of the modification
            validateCVModification(mod_part);

            ModificationAttributes attributes;
            attributes.modification_name = mod_part;
